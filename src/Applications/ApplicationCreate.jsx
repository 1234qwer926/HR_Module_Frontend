...
export default function ApplicationCreate() {
  ...
  const fetchJob = async () => {
    try {
      const response = await fetch(`https://mails-split-sec-units.trycloudflare.com/jobs/${jobId}`);
      if (response.ok) {
  ...
  const handleResumeUpload = async (file) => {
    ...
    try {
      ...
      const uploadResponse = await fetch('https://mails-split-sec-units.trycloudflare.com/upload-to-s3', {
        method: 'POST',
        body: formDataUpload,
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('https://mails-split-sec-units.trycloudflare.com/applications', {
        method: 'POST',
        headers: {
